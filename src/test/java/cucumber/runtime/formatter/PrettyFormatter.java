...
/**
 * Shadowed pretty cucumber.runtime.formatter class. Since version 4 cucumber supports parallel execution and PrettyFormatter needs to implements
 * ConcurrentEventListener.
 * If the ConcurrentEventListener is not implemented, the messages are not printed during test execution, but they are printed at the end at once.
 */
final class PrettyFormatter implements ConcurrentEventListener, ColorAware {
    ...
    private void handleTestStepStarted(TestStepStarted event) {
        if (event.testStep instanceof PickleStepTestStep) {
            if (isFirstStepAfterBackground((PickleStepTestStep) event.testStep)) {
                ...
                printStep((PickleStepTestStep) event.testStep, null);
            }
        }
    }
    ...
    private void printScenarioOutline(ScenarioOutline scenarioOutline) {
        ...
        out.println(SCENARIO_INDENT + getScenarioDefinitionText(scenarioOutline) + " " +
            getLocationText(currentFeatureFile, scenarioOutline.getLocation().getLine()));
    ...
    private void printStep(PickleStepTestStep testStep, Result result) {
        ...
        final String format = result == null ? "executing" : result.getStatus().lowerCaseName();
        String formattedStepText =
            formatStepText(keyword, stepText, formats.get(format), formats.get(format + "_arg"), testStep.getDefinitionArgument());
