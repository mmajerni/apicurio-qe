...
Feature: simpleTestMainPage
  Background:
      ...
      And click on the "New API" button
@basic
  Scenario: basic main page test
      ...
      And check that API consume "text/xml"
      And check that API produce "text/xml"
...
@search
  ...
  Scenario: fill all get parameters
    ...
    And set response type "String" formatted as "Password" for response 200

    And set response description "Description for response 100" for response 100
    And set response type "Number" formatted as "Float" for response 100
    ...
    And check that exist response 200 with description "Description for response 200" and type "String" as "Password"
    And check that exist response 100 with description "Description for response 100" and type "Number" as "Float"
  ...
  Scenario: create 2 data types with/without rest
    ...
    And set type "Integer" formatted as "32-Bit Integer" for property "MyProperty"
    ...
    #And check that exist response 200 with description "Successful response - returns an array of `data2` entities." and type "String" as "Password" #TODO need support for Array

    And check that operation "POST" is created for path "data2S"
    And select path "data2S"
    Then update operation "POST"
    ...
    #And check that exist response 201 with description "Successful response." and type "String" as "Password" #TODO need version without types
    ...
    And check that operation description is "Gets the details of a single instance of a data2."
     #And check that exist response 200 with description "Successful response - returns a single `data2`." and type "data2" as "Password" #TODO need version with one type

    And check that operation "PUT" is created for path "data2S/{data2Id}"
    And select path "data2S/{data2Id}"
    Then update operation "PUT"
    ...
    #And check that exist response 202 with description "Successful response." and type "String" as "Password" #TODO need version without types

    And check that operation "DELETE" is created for path "data2S/{data2Id}"
    And select path "data2S/{data2Id}"
    Then update operation "DELETE"
    ...
    And check that operation description is "Deletes an existing data2."
     #And check that exist response 204 with description "Successful response." and type "String" as "Password" #TODO need version without types
  ...
  Scenario: create path with parameters and one override
    ...
    And set type "String" formatted as "Byte" for parameter "myId"
    ...
    And set type "String" formatted as "Date" for parameter "id"

    And set description "parameter api desc" for path parameter "api"
    And set type "String" formatted as "Binary" for parameter "api"
